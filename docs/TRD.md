...
- Recently viewed, kept in `localStorage` on the device
- Methodology page, including a published account of what the score cannot see
- A 147-assertion test suite in the browser, plus twelve end-to-end checks against the live API
...
Data layer (all fetched or computed in the browser)
  - Product catalog: Open Pet Food Facts API — public, keyless, CORS
  - Additive Knowledge Base: committed JSON under assets/data/
  - AAFCO adult-maintenance minimums: constants in scoring.js
  - Scoring engine: pure ES module, runs client-side
```
...
├── compare.html             # Two products side by side; ?a= and ?b=
...
│   ├── js/                  # Application ES modules
│   │   ├── opff.js          #   API client + normaliser
...
|---|---|---|
| `index.html` | Built | Home: search form, Scan, recently viewed from local storage |
| `search.html` | Built | Live search results, scored; query via `?q=` |
| `product.html` | Built | Live product detail and score; `?barcode=` |
...
| Search | One page of 24 results, no pagination, no filters, no brand or food-type browse; API relevance discarded by a scorable-first re-sort | Pagination plus a browse structure over the top; keep relevance as the spine and make scorability a filter rather than a hidden sort key (M15) |
| Product images | `opff.js` requests and normalises `imageUrl`, and roughly 23 records in 24 carry one, but only `product-page.js` renders it | Render it on search cards, compare and recently viewed, with a placeholder for records that have none (M15) |
