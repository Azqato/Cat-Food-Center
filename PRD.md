...
Our team analyzes ingredients, nutritional value, and additives for every major brand on the market, scores each product on a transparent scale, and presents the result in seconds. Reviews are unbiased: no advertiser pays to change a score.
...
* Flag additives with real health concerns for cats and explain why, with sources.
* Work brilliantly on a phone in a store aisle, and equally well in a desktop browser.
...
* A persistent **header** on every screen contains the wordmark and a **Support** button linking to `https://azqato.github.io/support.html`.
...
The CFC Score is a 0 to 100 number with a color band. It is purpose-built for cats, because cats are obligate carnivores: they need animal-based protein for taurine and arginine, which they cannot make from plants, plus arachidonic acid, which they cannot synthesize from linoleic acid. Human food-scoring systems (such as Nutri-Score) are therefore not appropriate, and we do not use them.
...
Scored on dry-matter basis where data allows. Sub-factors:
...
* **Tier 3, high risk (triggers a score cap):**
  ...
  * BHA (butylated hydroxyanisole), BHT (butylated hydroxytoluene), ethoxyquin (synthetic preservatives linked to organ and carcinogenicity concerns in animal studies)
...
* **Propylene glycol detected:** automatic red flag and the product is capped in the Bad band, because it is FDA-prohibited in cat food.
* **Any Tier 3 additive present:** maximum possible score is 49 (Poor ceiling), regardless of nutrition. In this case the additive criterion can represent more than its base 35% weight.
...
| Band | Range | Color |
| --- | --- | --- |
| Excellent | 75 to 100 | Dark green |
| Good | 50 to 74 | Light green |
| Poor | 25 to 49 | Orange |
| Bad | 0 to 24 | Red |

### 6.7 Worked example

A semi-moist food whose first ingredient is corn, with unnamed meat by-products, added caramel color, and propylene glycol: propylene glycol triggers the hard gate, so the product lands in Bad (red) with a clear "contains an ingredient prohibited in cat food" callout, even before the nutrition math runs.
...
* Fast: product page interactive in under two seconds on a mid-range phone.
